 ...
 * a bearer token contained in the `Authorization` header field, `bearer_token`
 * body parameter, or `bearer_token` query parameter.
 *
...
Strategy.prototype.authenticate = function(req) {
  ...
  if (req.body && req.body.bearer_token) {
    if (token) { return this.fail(400); }
    token = req.body.bearer_token;
  }

  if (req.query && req.query.bearer_token) {
    if (token) { return this.fail(400); }
    token = req.query.bearer_token;
  }
